...
export function PersonTotals({
  ...
  return (
    <details className="group rounded-xl border border-primary/25 bg-surface p-3">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
        <span className="text-base font-bold">
          {person.name}
          {isOwn && (
            <span className="text-muted-foreground"> ({messages.you})</span>
          )}
        </span>
      ...
      <div className="mt-3 flex flex-col gap-1 border-t border-primary/20 pt-3 text-xs text-muted-foreground">
      ...
      {isOwn && onTogglePaid && (
        <button
          ...
          disabled={disabled}
          className="mt-3 w-full rounded-full border border-primary px-4 py-2 text-sm font-medium text-primary hover:bg-primary/10 disabled:opacity-50"
        >
